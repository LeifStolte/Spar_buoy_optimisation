...
# Fixed Length Fractions (Equal distribution among nodes)
...
def _split_design(x):
    """
    Splits the modified 12-element design vector into physical parameters.
    Injects the fixed uniform length fractions automatically.
    """
    x = np.asarray(x, dtype=float).reshape(-1)
    D_nodes = x[:N_NODES]
    draft = float(x[-2])
    ballast_mass = float(x[-1])
    return D_nodes, FIXED_L_FRACS, draft, ballast_mass
...
def _select_initial_design(waves, wind):
    ...
    candidates = [
          {
            'name': 'original',
            'D': [20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0, 20.0],
            'draft': 100.0,
            'ballast': 2.4e7,
        },
        {
    ...
    for cand in candidates:
        # Construct vector without L_fracs (12 elements total)
        x = np.array([*cand['D'], cand['draft'], cand['ballast']], dtype=float)
...
def run():
    ...
    # Combined bounds vector has 12 entries total now (no frac_bounds)
    bounds = d_bounds + draft_bounds + ballast_mass_bounds
    ...
    # Move limits mapped to match the 12 design parameters
