...
for i in range(len(winds)):
    ...
    q = ode4(dqdt, tode, q0, SparBuoyData, IEA22MWRotor, waves, wind, 'q19')
    ...
    fig19 = makeplots(wind, waves, SparBuoyData, response, timeInfo, colors[i], ax=ax)
    ax[0,0].legend([labels[i]])
    plt.tight_layout()
    plt.savefig(ofy(f"fig19{chr(97+i)}.pdf"))
    
    # Print the standard deviations
    print('--- Results for wind speed:', labels[i], '---')
    print(f'Q19 Surge Standard deviation [m]: {np.std(q[:,0])}')
    print(f'Q19 Pitch Standard deviation [deg]: {np.rad2deg(np.std(q[:,1]))}')
    ...
    print(f'Q19 Surge Std after TTrans [m]: {surge_std_post}')
    print(f'Q19 Pitch Std after TTrans [deg]: {pitch_std_post}')